...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['valider_achat'])) {
    ...
    if (strlen($numero_carte) === 16 && is_numeric($numero_carte)) {
        ...
        try {
            ...
            $stmt_update = $pdo->prepare("UPDATE annonces SET statut = 'VENDU' WHERE id = ?");
        ...
        $erreur = "Numéro de carte invalide (doit contenir 16 chiffres).";
    }
}
?>
...
<body>
    ...
    <div class="container-achat">
        <?php if (!$success): ?>
            ...
            <p>Total à payer : <strong><?= number_format($annonce['prix'], 2, ',', ' ') ?> €</strong></p>
            ...
            <form method="POST">
                ...
                <div class="form-group">
                    <label>Numéro de Carte Bleue (16 chiffres) :</label>
                    <input type="text" name="numero_carte" placeholder="0000 0000 0000 0000" maxlength="16" required>
                </div>
                ...
                <button type="submit" name="valider_achat" class="btn-valider">Confirmer le paiement</button>
            </form>

        <?php else: ?>
            <div class="facture">
                <h1 style="text-align: center;">FACTURE E-BAZAR</h1>
                <hr>
                ...
                <button onclick="window.print()" class="btn-valider">Imprimer la facture</button>
